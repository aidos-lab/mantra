...
def _suffix(balanced: bool, subdivision_level: int | None) -> str:
    """Filename suffix used by both raw and processed paths.

    ``subdivision_level=None`` means the original (un-subdivided) dataset;
    a positive integer encodes the number of barycentric subdivisions
    applied. The naming convention matches what the release artifacts
    are expected to be uploaded under.
    """
    parts = ["_balanced"] if balanced else []
    if subdivision_level is not None:
        parts.append(f"_bary{subdivision_level}")
    return "".join(parts)
...
def _get_dataset_url(
    ...
    suffix = _suffix(balanced, subdivision_level)
...
class ManifoldTriangulations(InMemoryDataset):
    ...
    def __init__(
        ...
        if subdivision_level is not None:
            assert (
                isinstance(subdivision_level, int) and subdivision_level >= 1
            ), f"subdivision_level must be a positive int, got {subdivision_level!r}"
        ...
        self.url = _get_dataset_url(
            version, dimension, balanced, subdivision_level
        )
    ...
    def raw_file_names(self):
        ...
        """
        suffix = _suffix(self.balanced, self.subdivision_level)
