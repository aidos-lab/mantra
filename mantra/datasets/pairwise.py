...
import torch
from mantra.datasets.base_dataset import ManifoldTriangulations
...
from mantra.transforms.structural_transforms import AddSimplicialComplexTransform, SetNumNodesTransform
...
class PairwiseSimplicialDS(InMemoryDataset):
    ...
    def __init__(
        self,
        root: str,
        comparison_pair: Tuple[Manifold3Type, Manifold3Type] | Tuple[Manifold2Type, Manifold2Type] = (Manifold2Type.S_2, Manifold2Type.T_2),
        manifold="2",
        version="latest",
        transform=None,
        pre_transform=None,
        ...
        self.load(self._get_processed_path(self.comparison_pair[0], self.comparison_pair[1]))
    ...
    def _data_filename(self, m_1: Manifold2Type | Manifold3Type, m_2: Manifold2Type | Manifold3Type):
        """
            Parameters
            -----------
                m_1: ManifoldType
    ...
    def _get_processed_path(self, m_1: Manifold2Type | Manifold3Type, m_2: Manifold2Type | Manifold3Type):
            ...
            for j, _m2 in enumerate(data_list_preprocessed):
                ...
                for k,v in _m1.to_dict().items():
                    if isinstance(v, torch.Tensor):
                        dict_obj[k] = torch.cat([v.unsqueeze(0), getattr(_m2, k).unsqueeze(0)], dim=0)
                    else:
