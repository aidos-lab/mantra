...
class PropertyPredictionDS(InMemoryDataset):
    ...
    def process(self):
        ...
        # Unreachable in practice: ``processed_file_names`` already raises
        # ``NotImplementedError`` for any non-2D manifold while computing
        # ``processed_paths``, i.e. before ``process`` is ever called.
        if self.manifold == "3":  # pragma: no cover
            raise NotImplementedError("TODO")
        ...
        if self.transform is not None:
            data_list_processed = [
                self.transform(d) for d in self.raw_simplicial_ds
            ]
        else:
            data_list_processed = [d for d in self.raw_simplicial_ds]
