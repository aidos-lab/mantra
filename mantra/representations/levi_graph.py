...
from mantra.representations.hasse_diagram import simplex_feature_index


class LeviGraph(BaseTransform):
    def __init__(self, feature_propagation: Optional[str] = None):
        super().__init__()
        # Name of a propagating embedding (e.g. `coordinate_embedding`)
        # whose per-rank tensors `<name>_0` ... `<name>_d` supply the
        # node features of the Levi graph: vertices carry their rank-0
        # feature, maximal simplices the feature of their own rank.
        # Mirrors the interface of `HasseDiagram` / `DualGraph`.
    ...
    def _build_levi(self, top_simplices, data):
        ...
        for i, simp in enumerate(top_simplices):
            ...
            if self.feature_propagation:
                ...
                attrs[self.feature_propagation] = feat_tensor[
                    feat_index[simp]
                ]
