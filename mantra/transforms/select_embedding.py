...
class SelectFeatures(BaseTransform):
    ...
    def __init__(self, src: str, dst: Union[str, List[str], None], representation: Representation = "graph"):
        ...
        assert representation in ["graph", "sc"], f"Invalid value: {representation}"
    ...
    def forward(self, data: Data):
        ...
        assert self.src in data, f"The `data` obj does not contain `{self.src}`"
        ...
        if isinstance(self.dst, List):
            assert isinstance(src_tensor, Dict), f"The attribute {self.src} is not of type dict"
            assert len(self.dst) == len(src_tensor.keys()), f"There is a mismatch between num of `src` keys ( {len(src_tensor.keys())} ) and `dst` targets ( { len(self.dst) } )"
        ...
        # representations for graph are PyG and indexed feature tensors based for simplicial complexes
            ...
            if self.representation == "graph":
                assert isinstance(src_tensor, Tensor), "Attribute `src` is not a `torch.Tensor`"
