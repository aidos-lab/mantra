...
class BettiToClassTransform(T.BaseTransform):
    ...
    def __init__(self, manifold_dim: int = 2):
        assert manifold_dim == 2 or manifold_dim == 3, "Only 2 and 3 manifolds are supported"
        self.manifold_dim = manifold_dim

    def forward(self, data: Data):
        data.y = torch.tensor(data.betti_numbers, dtype=torch.float).view(1, self.manifold_dim+1)
        return data
