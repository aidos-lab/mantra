...
class NodeRandomTransform(T.BaseTransform):
    ...
    def forward(self, data):
        if not self.propagate:
            # Prefer the explicitly-set ``num_nodes`` (works for bare
            # triangulations too); fall back to inferring from
            # ``edge_index`` if num_nodes isn't set.
            if data.num_nodes is not None:
                n = int(data.num_nodes)
            else:
                assert "edge_index" in data, (
                    "NodeRandomTransform needs either data.num_nodes or "
                    "data.edge_index to know how many nodes to sample for."
                )
                n = int(data.edge_index.max().item() + 1)
            data.random_features = torch.rand(size=(n, self.dimension))
