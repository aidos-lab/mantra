...
class TestAttributeToNodeClassTransform:
    ...
    def test_none_mapping_uses_values_as_indices(self):
        transform = AttributeToNodeClassTransform("label", None)

        result = transform(Data(label=torch.tensor([2, 0, 1])))

        assert result.y.dtype == torch.long
        assert result.y.tolist() == [2, 0, 1]

    def test_label_keeps_raw_values_through_batching(self):
        transform = AttributeToNodeClassTransform("label", self.MAPPING)

        batch = Batch.from_data_list(
            [
                transform(Data(label=torch.tensor([11, 3]))),
                transform(Data(label=[7])),
            ]
        )

        assert batch.label.tolist() == [11, 3, 7]
